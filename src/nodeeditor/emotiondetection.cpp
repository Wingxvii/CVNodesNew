...
void EmotionDetection::multiThreadedProcess()
{
    ...
    for(cv::Mat frame : videoIn->_video){
        ...
        LOG_JOHN() << "Frame: " + QString::fromStdString(std::to_string(dataOut->_valuePercentages.size()));

        for(std::pair<int, double> pair : valuePercentages){
            QString x = QString::fromStdString( dataOut->_labels[pair.first-1]) + ": " + QString::fromStdString(std::to_string(pair.second));
            LOG_JOHN() << x;
        }
